...
dwh <- d1 %>%
  ...
  full_join(e1_all_ages, by=c('state','date')) %>%
  full_join(g1_state, by=c('state','date')) %>%
    arrange(state, date) %>%
  group_by(state) %>%
  filter(date>='2023-07-01') %>%
...
write.csv(dwh,'./Data/plot_files/dwh_combined_plot1.csv')
...
write.csv(d1_state, './Data/plot_files/NSSP_detailed_states.csv')
write.csv(d1_all, './Data/plot_files/NSSP_detailed_counties.csv')
...
b2019 <- read.csv('./Data/pneumococcus/jiac058_suppl_supplementary_table_s2.csv') %>%
  group_by(State, sero) %>%
  summarize(N_cases=n()) %>%
  mutate(sero=as.factor(sero), mean_cases=max(N_cases,na.rm=T)) %>%
  ungroup() %>%
  group_by(State) %>%
...
write.csv(ipd1, './Data/plot_files/pneumococcus_ABCs_st_1998_2021.csv')
write.csv(b2019, './Data/plot_files/pneumococcus_serotype_by_state.csv')
...
## s2_pre13 differs from s2_pre in the years filtered. s2_pre13 is not
## used, but it looks like s2_pre is.
...
## df_vt and df_nvt do not appear to be used.


write.csv(s2, './Data/plot_files/changes_pneumococcal_infections_ABCs_st_1998_2021.csv')
...
ipd2 <- readRDS('./Data/pneumococcus/ABCs_st_1998_2021.rds') %>%
  rename(agec = "Age.Group..years.",
         year=Year,
         st=IPD.Serotype,
         N_IPD = Frequency.Count) %>%
...
uad <- readxl::read_excel('./Data/pneumococcus/SSUAD/ramirez_ofid_2025_ofae727.xlsx') %>%
  ...
  full_join(ipd2, by='st') %>%
  filter(!is.na(N_SSUAD) & !is.na(N_IPD))


write.csv(uad, './Data/plot_files/ramirez_ofid_joined_IPD.csv')
...
write.csv(vax_age, './Data/plot_files/ramirez_ofid_joined_IPD.csv')
write.csv(vax_urban, './Data/plot_files/ramirez_ofid_joined_IPD.csv')
write.csv(vax_insurance, './Data/plot_files/ramirez_ofid_joined_IPD.csv')
