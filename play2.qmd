...
The goal of this platform is to collect and present timely and informative data on population health from a variety of sources, including traditional public health surveillance data, electronic health records, sewage testing, internet search trends, population surveys, and modeled outputs. We are just getting started and will be adding many more datasets and disease endpoints in the coming months. This project is hosted by the Yale School of Public Health.
...
### read a list of input data 
ve_datasets <- readRDS('./Data/nirsevimab_eff/ve_datasets.rds')

# all the pull dates 
pull_dates_range <- sort(unique(ve_datasets$ve.main$pull_date))
...
#| context: server
 ...
 # ------------------------------------------------------
  ...
  output$plot_ve_main <- renderPlot({
    ...
    filtered_data <- data[data$pull_date == as.Date(input$date6), ] %>% 
      dplyr::select(-pull_date)
...
# ------------------------------------------------------
  ...
  #     filtered_data %>%
  #     mutate(
  #       VE.median = as.numeric(str_extract(VE.adjusted, "^[0-9\\.]+")),
  #   VE.lb = as.numeric(str_extract(VE.adjusted, "(?<=\\()[0-9\\.]+")),
  #   VE.ub = as.numeric(str_extract(VE.adjusted, "(?<=-)[0-9\\.]+")),
...
sliderTextInput("date6", "Data vintage date:",
                                     choices = pull_dates_range,
                                     selected = max(pull_dates_range),
                                     grid = TRUE,
                                     width = "100%"
                     )
```
...
We are [monitoring](https://www.medrxiv.org/content/10.1101/2024.09.12.24313545v1)the effectiveness of nirsevimab in Connecticut, using the latest data. You can see how our estimates of VE evolve as new data accrue. Counts are rounded to the nearest 10. Analysis led by Hanmeng Xu and Carlos Oliveira.
