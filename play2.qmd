...
url <- "https://github.com/DISSC-yale/gtrends_collection/raw/refs/heads/main/data/term=rsv/part-0.parquet"
...
g1_state <- read_parquet(temp_file) %>%
...
dwh <- d1 %>%
  ...
  left_join(g1_state, by=c('state','date')) %>%
    arrange(state, date) %>%
  group_by(state) %>%
  filter(date>='2023-07-01') %>%
  mutate(percent_visits_rsv_3m = zoo::rollmean(percent_visits_rsv, k = 3, fill = NA, align='right'),
        ...
        google_3m = zoo::rollmean(search_volume, k = 3, fill = NA, align='right'),
    ...
    percent_visits_rsv_3m = percent_visits_rsv_3m / max(percent_visits_rsv_3m, na.rm=T)*100,
       ...
       google_3m =google_3m/max(google_3m, na.rm=T)*100
...
output$distPlotRSV_dwh <- renderPlotly({
  ...
  geom_line(aes(x=date, y=google_3m,color='Google'), lty=1) +  
    ...
    scale_color_manual(name = "RSV", values = c("ED" = "#e41a1c", "WasteW"="#377eb8", 'Hosp'='#4daf4a', 'Epic'='#984ea3','Google'='#ff7f0070'  ))+
...
Lebanon, and Saudi Arabia. Epic data were obtained using the SlicerDicer tool in [Epic Cosmos.](https://www.epicresearch.org/about-us)
