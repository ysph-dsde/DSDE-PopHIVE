...
dwh <- d1 %>%
  ...
  mutate(percent_visits_rsv_3m = zoo::rollmean(percent_visits_rsv, k = 3, fill = NA, align='right'),
        ...
        google_3m_a = zoo::rollmean(search_volume, k = 3, fill = NA, align='right'),
        google_3m_b = zoo::rollmean(rsv_novax, k = 3, fill = NA, align='right'),
        google_3m_c = zoo::rollmean(rsv_novax2, k = 3, fill = NA, align='right'),
      ...
      rsv_ww_3m= rsv_ww_3m/max(rsv_ww_3m, na.rm=T)*100,
       ...
       google_3m_a =google_3m_a/max(google_3m_a, na.rm=T)*100,
       google_3m_b =google_3m_b/max(google_3m_b, na.rm=T)*100,
       google_3m_c =google_3m_c/max(google_3m_c, na.rm=T)*100,
       )

# dwh %>%
#   filter(state=='New York' & date>='2023-07-01') %>%
#   ggplot(aes(x=date, y=google_3m_b)) +
#     geom_line()+
#     geom_line(aes(x=date, y=google_3m_a), col='red') +
#   geom_line(aes(x=date, y=google_3m_c), col='blue') +
#   geom_line(aes(x=date, y=N_epic_3m), col='blue', linewidth=2) 
...
output$distPlotRSV_dwh <- renderPlotly({
  ...
  geom_line(aes(x=date, y=google_3m_b,color='Google'), lty=1) +  
  geom_line(aes(x=date, y=google_3m_c,color='Google (adj)'), lty=1) +  

            theme_minimal()+
    xlab('Date')+
    ylab('RSV activity (scaled to 100)')+
    scale_color_manual(name = "RSV", values = c("ED" = "#e41a1c", "WasteW"="#377eb8", 'Hosp'='#4daf4a', 'Epic'='#984ea3','Google'='#ff7f0070' ,'Google (adj)'='#a65628' ))+
