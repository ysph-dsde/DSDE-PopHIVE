...
output$distPlotflu_epic_ED_age <- renderPlotly({
  ...
  flu_epic_ts_comp <- epic_ed_combo %>%
    filter(geography==input$state.select4 & date >=as.Date('2023-07-01') & outcome_name=='FLU') %>%
...
output$distPlotcovid_epic_ED_age <- renderPlotly({
  ...
  covid_epic_ts_comp <- epic_ed_combo %>%
    filter(geography==input$state.select5 & date >=as.Date('2023-07-01') & outcome_name=="COVID") %>%
