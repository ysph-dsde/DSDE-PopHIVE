...
p1 <- usmap::plot_usmap(regions = "state", data = map_data, values = "Outcome_value3") +
...
output$distPlotflu_epic_ED_age <- renderPlotly({
  ...
  flu_epic_ts_comp <- epic_ed_combo %>%
    filter(geography==input$state.select4 & date >=as.Date('2023-07-01') & outcome_name=='FLU' & age_level !='Total') %>%
...
output$distPlotcovid_epic_ED_age <- renderPlotly({
  ...
  covid_epic_ts_comp <- epic_ed_combo %>%
    filter(geography==input$state.select5 & date >=as.Date('2023-07-01') & outcome_name=="COVID" & age_level !='Total') %>%
