...
summarized_opioid_df <- read_csv("./Data/plot_files/summarized_opioid_df.csv", show_col_types = FALSE)

waffle_df  <- read_csv("./Data/plot_files/opioid_waffle.csv", show_col_types = FALSE)
wonder_map <- read_csv("./Data/plot_files/opioid_ahrq_map.csv", show_col_types = FALSE)
ahrq_map   <- read_csv("./Data/plot_files/opioid_wonder_map.csv", show_col_types = FALSE)
...
output$distPlot1 <- renderPlotly({
  p1 <- summarized_opioid_df %>%
    dplyr::filter(dataset == 'AHRQ' & state == input$state.select) %>%
...
output$distPlot2 <- renderPlotly({
  p1 <- summarized_opioid_df %>%
    dplyr::filter(dataset == 'NCHS' & state == input$state.select) %>%
    ggplot(aes(x = qdate, y = count, group = level, color = level)) +
      theme_minimal() +
      geom_line()
...
output$opioid_time_series_interactive_2 <- renderPlotly({
  p <- summarized_opioid_df %>%
    # Filter out the yearly counts and only keep the quarterly counts.
    .[!str_detect(summarized_opioid_df$year_quarter, "NA"), ] %>%
