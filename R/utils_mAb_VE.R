...
##Round the count data to nearest 10
### main VE results
# ve.main <- readRDS("./Data/nirsevimab_eff/ve.main.rds") %>%
#   mutate(across(starts_with("n_"), ~ round(as.numeric(str_replace(., " \\s*\\([^\\)]+\\)", "")),-1))
#          ) 
# 
# ### waning VE
# ve.wane.monotrend <- readRDS("./Data/nirsevimab_eff/ve.biwkinterval.monotrend.rds")%>%
#   mutate(across(starts_with("n_"), ~ round(as.numeric(str_replace(., " \\s*\\([^\\)]+\\)", "")),-1))
#   ) 
# ve.wane.RW2 <- readRDS("./Data/nirsevimab_eff/ve.biwkinterval.RW2.rds")%>%
#   mutate(across(starts_with("n_"), ~ round(as.numeric(str_replace(., " \\s*\\([^\\)]+\\)", "")),-1))
#   ) 
# 
# 
...
# )
# 
# saveRDS(ve_datasets, './Data/nirsevimab_eff/ve_datasets.rds')
  ...
  for(i in 1:nrow(df.plot)){
    ...
    df.plot.rearranged.temp <- 
      data.frame(
        ...
        case = c(NA, round(as.numeric(df.plot.temp$n_cases_nomab,2)), round(as.numeric(df.plot.temp$n_cases_mab,2))),
